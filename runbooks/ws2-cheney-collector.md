...
A **minimum `LO_HEAP_SIZE` of `2 * HEAP_ALIGN`** (= 32 bytes on 64-bit) is enforced — splitting a smaller total into two semispaces yields halves that cannot hold even one 16-byte object header, so WS-1's `>= 16` floor is raised here to `>= 2 * HEAP_ALIGN`. Sub-floor or unparseable values fall back to the 16 MiB default (the unchanged WS-1 out-of-range behavior). The floor was raised consistently across all three skeletons during the build and **ratified by DC 2026-05-30**; see Resolved decision B.
...
**Cascade — reconciled 2026-05-30 (SC).** `.rodata` placement made ABI §2.3 and §3.6 stale where they described `LO_EMPTY_STRING` as allocated during `lo_runtime_init`. The ABI now reads consistently with the object model: §2.3 describes the read-only `.rodata` static object (symbol denotes the object, link-time-constant address, codegen embeds it with no load), §3.6 notes it needs no initialization, and the §4.4 "provided" list carries it as a fixed export. No further ABI edit is needed.
...
**Dependency cleared (WS-1 delta #4) — 2026-05-30.** The shared `tests/lo_programs/*.lo` fixtures previously carried best-effort, *unverified* LO-3 surface syntax (`TODO(SC)` per `runbooks/notes/for-sc-ws1-spec-deltas.md`); they were rewritten to grammar-valid LO-3 against `planning:lo-3-reference.md` (three-section class declarations, `int Main.main()` entry, the pre-bound `out` sink, snake_case IO methods). The `expected/*.out` files remain the stable contract. The harness consuming them is now a meaningful conformance signal, so WS-2 can be declared green once a clean checkout + Cheney runs the suite per skeleton.
...
**B — Heap size: inherits WS-1 (5c).** `LO_HEAP_SIZE` is read at `lo_runtime_init`, so the heap size is a runtime-overridable value (16 MiB default), not a compile-time constant; the collector inherits the existing WS-1 (5c) mechanism, with each semispace half the total (§2.1). The earlier "compile-time constant" framing was corrected 2026-05-30 once DC confirmed the override is consulted at runtime. Allocation-pressure tests set `LO_HEAP_SIZE` small to force collection deterministically. **Floor ratified 2026-05-30:** the minimum was raised from WS-1's `>= 16` to `>= 2 * HEAP_ALIGN` (= 32 on 64-bit) so each of the two semispaces can hold at least one 16-byte object header; sub-floor or unparseable values fall back to the 16 MiB default (§2.1).

**C — Forwarding-flag bit: CC's choice.** CC picks the `gc_bits` bit in Phase A and uses it consistently across all three skeletons (preserving the cross-skeleton byte-identical-header invariant). The bit is collector-private — no two menu collectors run on one heap — so it needs no ABI pin. **In the build, CC chose `gc_bits` bit 0** (`GC_FORWARDED_BIT` / `kGcForwardedBit`), the same bit in all three.

**Residual SC follow-up — discharged 2026-05-30.** Decision A's cascade into `runtime-abi.md` (§2.3 / §3.6 / §4.4) is reconciled to the object reading; see §2.5 Cascade above. No `TODO(SC)` remains here.

---

## Build outcome and ratified deltas (2026-05-30)

CC implemented the collector in all three skeletons (Rust reference in Phase A, Zig + C++ in Phase B; PRs `#10`, `#11`) and surfaced five points in `runbooks/notes/for-sc-ws2-spec-deltas.md`. SC's reconciliation, routed by DC:

- **Shared-ABI-symbol changes are atomic across the harness + all skeletons (delta #1) — runbook sharpened.** A change to the *meaning* of a shared ABI symbol (decision A flipped `LO_EMPTY_STRING` from an exported pointer *variable* to the exported *object*) cannot land in a Rust-only Phase A: the shared `tests/c_harness/main.c` consumes the symbol and the `cross_skeleton` CI job runs it against all three skeletons, and no single C declaration spans both models. Such a change is therefore **exempt from the otherwise-Rust-only Phase A** and must flip `main.c` + all three skeletons together. DC's mid-phase call (2026-05-30) applied exactly this: the trivial `.rodata` symbol flip went into Phase A for all three (collectors stayed stubbed in Zig/C++ until Phase B), keeping CI green throughout. Read §4's Phase A/B framing with this exemption in mind — it is the WS-2 analog of WS-1 delta #1.
- **`LO_HEAP_SIZE` floor `>= 2 * HEAP_ALIGN` (delta #3) — ratified.** See §2.1 and Resolved decision B.
- **Allocation-pressure test injection differs by language (delta #4) — soft-consistency record.** The Rust A.4 test (v) forces a small heap via the real `LO_HEAP_SIZE` env path; Zig and C++ use a non-ABI test hook (`heapInitWith` / `heap_init_with`) taking an explicit total, because setting the C environment before the runtime's `getenv`/`getEnvVarOwned` would mean linking libc into the Zig test binary and is racy across the test runners. The env-reading logic itself is identical across all three; only the test's injection differs, and the env path is covered by the Rust reference. Behavior is identical — recorded as a soft-consistency item (the WS-1 delta #5 ledger), no spec change.
- **ABI text reconciliation (delta #2) — discharged.** See §2.5 Cascade and the Residual note above.

**Conformance gate (delta #5 — §4) — unblocked.** The cross-skeleton C harness is green for all three skeletons with Cheney (ABI link + behavior witness; they agree on `42`). The shared `tests/lo_programs/*.lo` fixtures, previously best-effort `TODO(SC)` placeholders, were rewritten to grammar-valid LO-3 against `planning:lo-3-reference.md` (2026-05-30), so the `.lo`-driven conformance layer is now a meaningful signal. WS-2 goes green once a clean checkout + Cheney runs the suite per skeleton (the §4 acceptance bar).
